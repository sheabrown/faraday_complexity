...
class inception(loadData, plots, analysis):
	...
	def _compile(self, classes=2, l2reg=0.0002, act='softmax', optimizer='adadelta', loss='binary_crossentropy', metrics=['binarcy_accuracy']):
		"""
		Compiles the model by appending a Dense layer with
		"classes" outputs.

		To call:
			_compile(classes, l2reg, ...)

		Parameters:
			classes:	number of outputs
			l2reg:		regularization parameter
		"""
		# ==================================================
		#	Add the final dense layer subject
		#	to an l2 regularization penalty 
		# ==================================================
		self.model_.append(Dense(classes, W_regularizer=l2(l2reg))(self.model_[-1]))
	...
	def _dense(self, z, act='relu', drop=0.5, ntimes=1, l2reg=0.0002):
		"""
		Function for adding a dense layer to the model.

		To call:
			_dense(z, act, drop, ntimes, l2reg)
		...
		Parameters:
			...
			l2reg		L2 regularization penalty
		"""
		# ==================================================
		...
		for _ in range(ntimes):
			...
			# ==================================================
			self.model_.append(Dense(z, W_regularizer=l2(l2reg))(self.model_[-1]))
	...
	def _inception(self, convl=[3, 5], pool=[3], act='relu', l2reg=0.0002):
		...
		try:
			...
			# ===========================================
			convl_1x1 = Conv1D(32, kernel_size=1, W_regularizer=l2(l2reg))(self.model_[-1])
			...
			# ===========================================
			for c in convl:
				convl_cx1 = Conv1D(64, kernel_size=c, strides=1, padding=padding, activation=act, W_regularizer=l2(l2reg))(convl_1x1)
			...
			for p in pool:
				...
				pconvl_1x1 = Conv1D(64, kernel_size=1, padding=padding, activation=act, W_regularizer=l2(l2reg))(pool_px1)
			...
			#	concatenated to the final output
			# ===========================================			
			convl_1x1 = Conv1D(32, kernel_size=1, input_shape=self.__inputShape, W_regularizer=l2(l2reg))(self.__input)
			model.append(convl_1x1)
